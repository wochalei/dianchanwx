<template>
	<view>
		...
		<view class="people-view">
			...
			<view class="table-num">桌号：{{tableName}}</view>
			<!-- 滑动 -->
				...
				<scroll-view scroll-x="true" class="scroll-view_H" :enable-flex="true">
					<view class="table-block">
						<block v-for="(item,index) in people" :key="index">
							<view :class="{activetext:cIndex===index}" @click="chooseMan(index)">{{item}}</view>
						</block>
					</view>
				</scroll-view>
			</view>
			<!-- 按钮 -->
			<view class="start-diancan" :class="{'start-activ':cIndex!=-1}" @click="toFood">开始点餐</view>
		</view>
	</view>
</template>
